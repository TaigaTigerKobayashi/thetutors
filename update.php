...
$sql = 'UPDATE calendar_table SET STUDENT_ID=:STUDENT_ID,TUTOR_ID=:TUTOR_ID,title=:title,text=:text,day=:day,start=:start,end=:end WHERE id=:id';
//prepare("")の中にはmysqlのSQLで入力したINSERT文を入れて修正すれば良いイメージ

//3-2: sql文をstmtに渡す処理
$stmt = $pdo->prepare($sql);

//3-3: 関連付けをして、STUDENT_IDやemailを3-1の同じ文字に紐付ける(ここはinsert.phpから修正している)
$stmt->bindValue(':STUDENT_ID',   $STUDENT_ID,   PDO::PARAM_STR);  //Integer（数値の場合 PDO::PARAM_INT)  第３引数は省略出来るが、セキュリティの観点から記述している。文字列か数値はmysqlのデータベースに登録したものがvarcharaかintかというところで判断する
$stmt->bindValue(':TUTOR_ID', $TUTOR_ID, PDO::PARAM_STR);  //Integer（数値の場合 PDO::PARAM_INT)
...
if($status==false){
  ...
  header('Location: student_list2.php');//Location:の後ろの半角スペースは必ず入れる。
  exit;
